...
def _normalize_anchor_tag_value(value: str) -> str:
    """Нормализует значение тега якоря для точного сравнения."""
...
class CallHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        # Принимаем стандартные записи fs_*, external-*, новый формат вход_* и формат без префикса; расширения из конфигурации
...
def _build_strict_anchor_prompt(prompt: str) -> str:
    prompt = re.sub(
        r"Если\s+Не\s+Целевой\s*,?\s*дополнительн\w*\s+анализ\s+не\s+нужен\.?",
        "Если звонок Не Целевой, всё равно верни обязательные теги для нецелевого звонка.",
        prompt or "",
        flags=re.IGNORECASE,
    )
    strict_suffix = """

ОБЯЗАТЕЛЬНЫЙ ФОРМАТ ОТВЕТА. Соблюдай его даже для нецелевого звонка.
Первые три строки ответа должны быть только такими тегами:
...
Если звонок нецелевой, всё равно верни:
[ТИПЗВОНКА:НЕЦЕЛЕВОЙ]
[КЛАСС:НЕТ]
[РЕЗУЛЬТАТ:НЕТ]

После тегов добавь строку "Объяснение:" и 2-3 коротких предложения.
Не добавляй markdown, списки, приветствия, английский текст и дополнительные теги.
...
Объяснение: клиент впервые обратился с вопросом по услуге и уточнял возможность решения. Записи не было, поэтому результат НЕТ.

Пример нецелевого ответа:
[ТИПЗВОНКА:НЕЦЕЛЕВОЙ]
[КЛАСС:НЕТ]
[РЕЗУЛЬТАТ:НЕТ]

Объяснение: клиент продолжал ранее начатый вопрос или уточнял статус текущих работ. Это не первичный запрос.
"""
        ...
        repair_messages = [
            {
                ...
                "content": (
                    ...
                    "Всегда верни [КЛАСС:...]; для нецелевого используй [КЛАСС:НЕТ] и [РЕЗУЛЬТАТ:НЕТ]. "
            ...
            {"role": "assistant", "content": analysis_text},
            {
                ...
                "content": (
                    ...
                    "Если звонок нецелевой, верни [ТИПЗВОНКА:НЕЦЕЛЕВОЙ], [КЛАСС:НЕТ], [РЕЗУЛЬТАТ:НЕТ]."
                ),
            },
        ]
...
def is_target_call(analysis_text: str) -> bool:
    ...
    if type_match:
        call_type = _normalize_anchor_tag_value(type_match.group(1))
        if call_type.startswith("НЕЦЕЛЕВОЙ"):
            return False
        return call_type in {"ЦЕЛЕВОЙ", "ПЕРВИЧНЫЙ"}
