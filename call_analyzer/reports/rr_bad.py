...
def send_telegram_report(file_path, text_message, config):
    ...
    if not token or not chat_ids:
        print('Telegram не настроен для rr_bad, сообщение пропущено.')
        return
...
ANSWER_REGEX = re.compile(
    r'(\d+)\.\s.*?\[ОТВЕТ:\s*(ДА|НЕТ)\]',
    flags=re.IGNORECASE | re.DOTALL
)
ANALYSIS_REGEX = re.compile(
    r'Результат анализа:\s*([\d\.]+)%',
    flags=re.IGNORECASE
)

###############################################################################
# Логика чтения логов tg_retruck_calls.txt (номер+дата -> фамилия)
###############################################################################
...
###############################################################################
# Сбор данных по txt-файлам (папки за неделю)
###############################################################################
...
###############################################################################
# Создаем отчёт только для тех, у кого < 60%
###############################################################################
def create_weekly_report_bad_calls(data_list, output_folder, last_monday, last_sunday):
    bad_calls = [item for item in data_list if item["analysis_percent"] < 60]
    if not bad_calls:
        print("Нет звонков с результатом анализа ниже 60%.")
        return None

    report_filename = f"Ретрак_Отчёт_Плохие_{last_monday.strftime('%d.%m')}-{last_sunday.strftime('%d.%m')}.xlsx"
    ...
    for stn in df_srt["станция"].unique():
        ws_summary.cell(row=row_summary, column=1, value=f"Станция: {stn}")
        ws_summary.cell(row=row_summary, column=1).font=bold_font
        row_summary+=1

        for col_i, hd in enumerate(headers, start=1):
            cc = ws_summary.cell(row=row_summary, column=col_i, value=hd)
            cc.font = bold_font
            cc.alignment = center_alignment
            cc.border = thin_border
        row_summary+=1

        subdf = df_srt[df_srt["станция"]==stn]
        for _, rdata in subdf.iterrows():
            col_i=1
            for hd in headers:
                val = rdata[hd]
    ...
    # Добавляем список вопросов в конце "Сводный отчёт"
    questions = [
    ...
    wb.save(full_path)
    print(f"Отчет с плохими звонками (<60%) сохранен: {full_path}")
    return full_path
...
def run_rr_bad():
    ...
    last_monday, last_sunday = get_last_week_dates()
    period_text = f"Отчет (плохие) за {last_monday.strftime('%d.%m')} - {last_sunday.strftime('%d.%m')}"

    calls_data = collect_weekly_data(base_folder, tg_calls)
    if not calls_data:
        print("Нет файлов (10 ответов) за этот период.")
        return
