...
_NAME_STOPWORDS = set([
    ...
    # Частые слова, ошибочно определяемые как имена
    "раз", "два", "три", "тут", "там", "вот", "все", "всё", "вам", "нас", "они", "сто",
    "час", "год", "лет", "еще", "ещё", "уже", "при", "под", "над", "без", "про", "для",
    "она", "они", "его", "ему", "них", "вон", "вот", "ведь", "пока", "если", "хотя",
    "куда", "тоже", "даже", "после", "перед", "через", "между", "около",
...
"""
    ...
    # Дополним общий промпт критериями по каждому пункту чек-листа.
    # Критерий из промпта пункта — главный для оценки этого пункта (не только «подсказка»).
    if checklist_titles:
        hints_lines = [
            "Правила оценивания (строго соблюдать):",
            f"1) Учитывай реплики обоих участников, если в критерии пункта не указано иное (например «только менеджер»).",
            "2) Для КАЖДОГО пункта применяй ТОЛЬКО критерий из блока «Критерий пункта» ниже — это главное правило. Название пункта служит только для идентификации; не подменяй критерий своей интерпретацией названия.",
            "3) Правило «при сомнении — НЕТ» применяй только когда критерий пункта не даёт однозначного ответа. Если в критерии явно сказано, при каких условиях ставить ДА (в т.ч. «даже если…») — следуй критерию.",
            "4) Формат ответа — одна строка на пункт в порядке 1..N:",
            f"   'i. <Название пункта> [ОТВЕТ: ДА|НЕТ] [ОБОСНОВАНИЕ: кратко, со ссылкой на критерий и диалог]'.",
            "5) Для пункта «Выявление потребности» (если есть) считай ДА только если есть ≥2 вопроса менеджера и ≥1 открытый вопрос или перефраз.",
            "6) Для пункта «Инициативность менеджера» (если есть) считай ДА только если есть явное предложение следующего шага.",
            "Критерии по пунктам (применять строго; они имеют приоритет над общими правилами для данного пункта):"
        ]
